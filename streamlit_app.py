...
with col1:
    ...
    fig = px.bar(levels,  title='Delegates by Class;', color_continuous_scale='darkmint', labels={'value': 'Count', 'Study_year': 'Class_level'})
    st.plotly_chart(fig)
with col2:
    school = ivsa['University '].value_counts().sort_values()
    fig = px.bar(school, color='red', title='Delegates by School;', color_continuous_midpoint=50, color_continuous_scale='sunset', labels={'value': 'Count',})
    st.plotly_chart(fig)
