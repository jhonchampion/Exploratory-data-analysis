...
with col1:
    level = ivsa['Study_year'].value_counts().sort_values()
    fig = px.bar(level, title='Delegates by Class;', color_continuous_scale='inferno', labels={'value': 'Count', 'Study_year': 'Class_level'})
    st.plotly_chart(fig)
with col2:
